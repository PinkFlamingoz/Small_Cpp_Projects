...
class Queue
{
	...
	void enqueue(Node<K, D> *new_node)
	{
		...
		if (is_empty())
		{
			front = new_node;
			rear = new_node;
			cout << "New node with key[" << new_node->get_key() << "] and data: " << new_node->get_data() << " enqueued successfully!" << endl;
		}
		else
		{
	...
	void dequeue()
	{
		...
		if (front == rear) //----------------------------------------------------------------------------------------- If we have one element in the queue
		{
			...
			cout << "Node with key[" << temp->get_key() << "] and data: " << temp->get_data() << " dequeued!" << endl;
			delete temp;
		}
