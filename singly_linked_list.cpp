...
class Node
{
	...
	// Get the pointer that points to the next node
...
class Singly_linked_list
{
	...
	void append_node(Node<K, D> *new_node)
	{
		...
		else //------------------------------------------------------------------------------------ CASE 2: There are already elements in the list so we traverse till the end of the list and we make the last element point at the new node
		{
	...
	void insert_node_sorted(Node<K, D> *new_node)
	{
		...
		{
			new_node->set_next(head); //-------------------------------------------------------------- Set the new node's next field, to point at the current head node
	...
	void delete_node_by_key(K key)
	{
		...
		while (current != nullptr)
		{
			if (current->get_key() == key)
			{
				...
				{
					previous->set_next(current->get_next());  //----------------------- We set the previous node pointer to point at the node that the current pointer points at
				}
