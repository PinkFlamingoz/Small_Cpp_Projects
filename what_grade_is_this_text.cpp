...
int count_words(string text)
{
	...
	{ // We use this to skip over if we have one or more spaces in the beginning of a sentence.
		i++;
	}
	...
	for (i; i < lenght; i++)
	{
		...
		{ // We use this to skip over multiple spaces in between words. We also get a true here if both are not spaces, because we get false == false, but it doesnt matter.
			count += 0;
		}
		else if (isspace(text[i]) && (i != lenght - 1))
		{ // We count the space thats in between words only if that space is not the last space of the sentence. We do this for example if the user typed a extra spaces at the end of a sentence.
			count++;
		}
	}
...
int count_sentences(string text)
{
	...
	{
		if (text[i] == text[i + 1])
		{ // We use this to skip over if we have multiple ... or ??? or !!! in one line.
			count += 0;
		}
		else if ((text[i] - text[i + 1]) == 13)
		{ // A very non effcient way to check if we have the sequence of !. = -13; .?=-17; !?=-30; also the opposites, so if we have in a sentece ?!? it doenst count them as 3 seperate but only one.
			count += 0;
		}
